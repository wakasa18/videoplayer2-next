import { AlertTriangle, Link2, ShieldX } from "@/components/ui/icons";
...
export default async function PublicSharePage({ params, searchParams }: PageProps) {
  ...
  return (
    <main className="grid min-h-screen place-items-center bg-card p-5 text-slate-100">
      <section className="relative w-full max-w-xl overflow-hidden rounded-[28px] border border-primary/10 bg-card p-7 text-center shadow-[0_28px_90px_rgba(0,0,0,.45)] sm:p-10">
        ...
        <div className="mt-6 inline-flex items-center gap-2 rounded-full border border-primary/10 bg-card px-4 py-2 text-xs font-semibold text-slate-400"><Link2 className="size-4 text-primary" /> Damon&apos;s Archive secure sharing</div>
      </section>
    </main>
  );
}
