import { AlertTriangle, Clock3, FileClock } from "@/components/ui/icons";
    ...
    return (
      <main className="space-y-5">
        <section className="tech-panel relative overflow-hidden rounded-[28px] p-6 sm:p-8">
          ...
          <div className="mb-4 inline-flex items-center gap-2 rounded-full bg-primary/10 px-3 py-1.5 text-xs font-semibold text-primary"><Clock3 className="size-4" /> Recently accessed</div>
        ...
        {files.length ? <FileGrid files={files} /> : <div className="tech-panel grid min-h-72 place-items-center rounded-[26px] p-8 text-center"><div><FileClock className="mx-auto size-10 text-primary" /><h2 className="mt-4 text-lg font-semibold text-slate-100">No recent files yet</h2><p className="mt-2 text-sm text-slate-400">Open or preview a file and it will appear here.</p></div></div>}
      </main>
    );
  } catch (error) {
