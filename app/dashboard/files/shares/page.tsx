import { ArrowLeft, Link2, ShieldCheck } from "@/components/ui/icons";
  ...
  return (
    <main className="space-y-5">
      <section className="tech-card overflow-hidden rounded-[28px] border border-white/10 bg-white/[0.045] p-6 shadow-sm sm:p-8">
        <Link href="/dashboard/files" className="inline-flex items-center gap-2 rounded-full px-3 py-2 text-sm font-semibold text-primary transition hover:bg-primary/10"><ArrowLeft className="size-4" /> Back to files</Link>
        <div className="mt-5 flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
          <div>
            <div className="mb-4 inline-flex items-center gap-2 rounded-full bg-primary/10 px-3 py-1.5 text-xs font-semibold text-primary"><ShieldCheck className="size-4" /> Secure public sharing</div>
      ...
      <div className="flex items-center gap-2 rounded-[18px] border border-primary/20 bg-primary/10 p-4 text-sm text-primary"><Link2 className="size-5 shrink-0" /><p>Create a new link from the three-dot menu on any file or folder.</p></div>
