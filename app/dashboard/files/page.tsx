...
function Summary({ label, value }: { label: string; value: string }) { return <div className="min-w-24 rounded-2xl border border-white/10 bg-white/[0.035] px-4 py-3"><span className="block text-xs font-medium text-slate-400">{label}</span><strong className="mt-1 block truncate text-lg font-semibold text-slate-100">{value}</strong></div>; }
function SectionHeading({ eyebrow, title, count }: { eyebrow: string; title: string; count: string }) { return <div className="flex items-center justify-between gap-3"><div><p className="text-xs font-bold uppercase tracking-[.08em] text-slate-400">{eyebrow}</p><h2 className="mt-1 text-lg font-semibold text-slate-100">{title}</h2></div><span className="rounded-full bg-white/[0.05] px-3 py-1.5 text-xs font-semibold text-slate-400">{count}</span></div>; }
