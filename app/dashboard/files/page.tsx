import { AlertTriangle, ChevronRight, FolderOpen, HardDrive, SearchX } from "@/components/ui/icons";
...
export default async function FilesPage({ searchParams }: FilesPageProps) {
  ...
  return <main className="space-y-5">
    ...
    {!filters.favorite ? <nav aria-label="Folder breadcrumb" className="tech-panel flex items-center gap-1 overflow-x-auto rounded-[18px] px-3 py-2 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">{result.breadcrumbs.map((crumb, index) => <span key={crumb.path || "root"} className="flex items-center gap-1">{index > 0 ? <ChevronRight className="size-4 shrink-0 text-slate-500" /> : null}<Link href={buildFileQuery(filters, { folder: crumb.path, page: 1 })} aria-current={index === result.breadcrumbs.length - 1 ? "page" : undefined} className={`whitespace-nowrap rounded-full px-3 py-2 text-sm font-medium transition ${index === result.breadcrumbs.length - 1 ? "bg-primary/10 text-primary" : "text-slate-400 hover:bg-white/[0.06]"}`}>{crumb.label}</Link></span>)}</nav> : null}

    <FilesToolbar filters={filters} categories={result.categories} />
    {result.truncated ? <div className="flex items-start gap-3 rounded-[18px] border border-amber-300/25 bg-amber-400/10 p-4 text-sm text-amber-300"><AlertTriangle className="mt-0.5 size-5 shrink-0" /><p>For performance, this view loads up to 5,000 active records at a time. Use search and filters to narrow large archives.</p></div> : null}
...
function EmptyFiles({ filters }: { filters: FileBrowserFilters }) { const filtered = Boolean(filters.q || filters.type || filters.category || filters.favorite); return <div className="grid min-h-72 place-items-center rounded-[24px] border border-dashed border-primary/20 bg-white/[0.045] p-8 text-center"><div className="max-w-md"><span className="mx-auto grid size-16 place-items-center rounded-2xl bg-primary/10 text-primary">{filtered ? <SearchX className="size-7" /> : <FolderOpen className="size-7" />}</span><h3 className="mt-5 text-lg font-semibold text-slate-100">{filtered ? "No matching files" : "This location is empty"}</h3><p className="mt-2 text-sm leading-6 text-slate-400">{filtered ? "Clear a filter or use a different search term." : "Upload files or create a folder to get started."}</p></div></div>; }
