import { ArrowLeft, Film, Trash2 } from "@/components/ui/icons";
...
export default async function VideoRecyclePage() {
  ...
  return (
    <main className="space-y-5">
      ...
      {result.videos.length ? <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-4">{result.videos.map((video, index) => <VideoCard key={video.id} video={video} index={index} recycled />)}</div> : <div className="grid min-h-72 place-items-center rounded-[24px] border border-dashed border-primary/20 bg-white/[0.045] p-8 text-center"><div><span className="mx-auto grid size-16 place-items-center rounded-2xl bg-primary/10 text-primary"><Film className="size-7" /></span><h2 className="mt-5 text-lg font-semibold text-slate-100">The Recycle Bin is empty</h2><p className="mt-2 text-sm text-slate-400">Deleted videos will appear here.</p></div></div>}
