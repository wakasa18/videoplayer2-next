    ...
    return (
      <main className="grid min-h-[68vh] place-items-center">
        <section className="w-full max-w-2xl rounded-[28px] border border-amber-300/25 bg-white/[0.045] p-8 shadow-sm">
          ...
          <div className="mt-5 rounded-2xl bg-white/[0.035] p-4 text-sm leading-6 text-slate-200">Run <code>database/phase6_videos.sql</code>, create the private video bucket, and configure <code>SUPABASE_VIDEOS_BUCKET</code>.</div>
        </section>
      </main>
    );
  }
      ...
      <section className="space-y-3">
        ...
        {result.videos.length ? (
          filters.view === "grid" ? (
            ...
            <div className="tech-panel overflow-hidden rounded-[24px]">{result.videos.map((video, index) => <VideoListRow key={video.id} video={video} index={index} />)}</div>
          )
