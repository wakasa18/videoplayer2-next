...
export const metadata = { title: "Release Handoff" };
...
export default async function HandoffPage() {
  try {
            ...
            <div className="flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
              <div className="max-w-3xl">
                <div className="mb-4 inline-flex items-center gap-2 rounded-full bg-primary/10 px-3 py-1.5 text-xs font-semibold text-primary">
                  <BadgeCheck className="size-4" /> Release acceptance and operational handoff
                </div>
                <h1 className="text-3xl font-semibold tracking-[-.03em] text-slate-100 sm:text-4xl">Release handoff center</h1>
    ...
    return <main className="grid min-h-[68vh] place-items-center"><section className="tech-panel w-full max-w-2xl rounded-[28px] p-8"><AlertTriangle className="size-8 text-amber-300" /><h1 className="mt-5 text-2xl font-semibold text-slate-100">Release Handoff database setup is required</h1><p className="mt-3 text-sm leading-6 text-slate-400">{error instanceof Error ? error.message : "Release Handoff data could not be loaded."}</p><div className="mt-5 rounded-2xl bg-white/[0.035] p-4 text-sm text-slate-200">Run <code>database/phase12_acceptance_handoff.sql</code> in Supabase, then refresh.</div></section></main>;
  }
}
