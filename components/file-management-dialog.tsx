    ...
    <AnimatePresence>
      {mode ? (
        ...
        >
          <motion.form
            onSubmit={submit}
            className="w-full max-w-xl overflow-hidden rounded-[28px] border border-[#e1e5ea] bg-white shadow-2xl"
          ...
          >
            <div className="flex items-start gap-4 border-b border-[#eef1f3] p-5 sm:p-6">
              <span className={`grid size-11 shrink-0 place-items-center rounded-2xl ${mode === "trash" ? "bg-[#fce8e6] text-[#c5221f]" : "bg-[#e8f0fe] text-[#1967d2]"}`}>
                {mode === "edit" ? <FilePenLine className="size-5" /> : mode === "move" ? <FolderInput className="size-5" /> : <Trash2 className="size-5" />}
              </span>
              <div className="min-w-0 flex-1">
                <h2 className="text-lg font-semibold text-[#202124]">{heading}</h2>
                <p className="mt-1 truncate text-sm text-[#80868b]">{file.original_filename}</p>
              </div>
              <button type="button" onClick={onClose} disabled={submitting} className="grid size-10 place-items-center rounded-full text-[#5f6368] transition hover:bg-[#f1f3f4] disabled:opacity-50">
            ...
            <div className="space-y-4 p-5 sm:p-6">
              ...
              {mode === "move" ? (
                <Field label="Destination">
                  ...
                  {loadingFolders ? <span className="mt-2 flex items-center gap-2 text-xs text-[#80868b]"><Loader2 className="size-3.5 animate-spin" /> Loading folders…</span> : null}
                </Field>
              ) : null}

              {mode === "trash" ? (
                <div className="flex gap-3 rounded-2xl border border-[#f6c7c3] bg-[#fce8e6] p-4 text-sm leading-6 text-[#8c1d18]">
            ...
            <div className="flex flex-col-reverse gap-2 border-t border-[#eef1f3] p-5 sm:flex-row sm:justify-end">
              <button type="button" onClick={onClose} disabled={submitting} className="min-h-11 rounded-full border border-[#dadce0] bg-white px-5 text-sm font-semibold text-[#3c4043] hover:bg-[#f8f9fa] disabled:opacity-50">Cancel</button>
              <button type="submit" disabled={submitting || loadingFolders} className={`inline-flex min-h-11 items-center justify-center gap-2 rounded-full px-5 text-sm font-semibold text-white disabled:opacity-60 ${mode === "trash" ? "bg-[#d93025] hover:bg-[#b3261e]" : "bg-[#1a73e8] hover:bg-[#1557b0]"}`}>
...
function Field({ label, children }: { label: string; children: React.ReactNode }) {
  return <label className="block"><span className="mb-2 block text-xs font-semibold text-[#5f6368]">{label}</span>{children}</label>;
}
