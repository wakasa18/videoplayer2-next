...
import { Check, CheckSquare, Download, Star } from "@/components/ui/icons";
...
function FileListRow({ file, index, selected, onSelectedChange }: { file: ImportantFile; index: number; selected: boolean; onSelectedChange: (selected: boolean) => void }) {
  ...
  async function toggleFavorite() {
    try {
      ...
      router.refresh();
    } catch (error) { (await showNotice(error instanceof Error ? error.message : "Could not update favorite status.")); }
  }
  ...
  return <>
    ...
    >
      <article draggable onDragStartCapture={(event) => { event.dataTransfer.effectAllowed = "move"; event.dataTransfer.setData("application/x-damons-file", JSON.stringify({ id: file.id, title: file.title })); }} style={{ "--card-reveal-delay": `${Math.min(index, 10) * 14}ms` } as React.CSSProperties} className={`tech-card-reveal grid grid-cols-[38px_minmax(0,1fr)_auto] items-center gap-3 bg-card/60 px-4 py-3 transition hover:bg-white/[0.04] md:grid-cols-[38px_minmax(0,1.6fr)_minmax(120px,.65fr)_minmax(130px,.7fr)_44px] md:gap-4 md:bg-transparent md:px-5 ${selected ? "bg-primary/[.055]" : ""}`}>
        <button type="button" aria-label={selected ? `Deselect ${file.title}` : `Select ${file.title}`} onClick={() => onSelectedChange(!selected)} className={`grid size-8 place-items-center rounded-xl border ${selected ? "border-primary/50 bg-primary text-[#122f29]" : "border-white/10 bg-white/[.035] text-transparent hover:text-slate-300"}`}><Check className="size-4" /></button>
        <button type="button" data-no-glass className="flex min-w-0 items-center gap-3 text-left" onClick={() => canPreviewFile(file) && setPreviewOpen(true)}><FileTypeIcon file={file} className="size-11 rounded-xl" iconClassName="size-5" /><span className="min-w-0"><span className="flex min-w-0 items-center gap-1.5"><strong className="min-w-0 flex-1 truncate text-sm font-semibold text-slate-100">{file.title}</strong>{file.is_favorite ? <Star className="size-3.5 shrink-0 fill-amber-300 text-amber-300" /> : null}</span><span className="mt-0.5 block truncate text-xs text-slate-400 md:hidden">{formatBytes(file.file_size)} · {formatDate(file.updated_at ?? file.created_at)}</span><span className="mt-0.5 hidden truncate text-xs text-slate-400 sm:block">{file.original_filename}</span></span></button>
