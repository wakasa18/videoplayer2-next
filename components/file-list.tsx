...
import { Check, CheckSquare, Star } from "lucide-react";
...
function FileListRow({ file, index, selected, onSelectedChange }: { file: ImportantFile; index: number; selected: boolean; onSelectedChange: (selected: boolean) => void }) {
  ...
  return <>
    <motion.article draggable onDragStartCapture={(event) => { event.dataTransfer.effectAllowed = "move"; event.dataTransfer.setData("application/x-damons-file", JSON.stringify({ id: file.id, title: file.title })); }} className={`grid grid-cols-[38px_minmax(0,1fr)_auto] items-center gap-3 px-4 py-3 transition hover:bg-white/[0.04] md:grid-cols-[38px_minmax(0,1.6fr)_minmax(120px,.65fr)_minmax(130px,.7fr)_44px] md:gap-4 md:px-5 ${selected ? "bg-cyan-300/[.055]" : ""}`} initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: Math.min(index, 20) * .025, duration: .2 }}>
