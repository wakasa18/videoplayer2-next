...
import { Archive, Loader2, Pencil, Plus, RotateCcw, Settings2, Trash2, X } from "@/components/ui/icons";
...
export function SubjectManagerDialog({
  ...
  const [color, setColor] = useState("#8fd6c2");
  ...
  useEffect(() => {
    ...
    setSchedule(selected?.schedule ?? ""); setSemester(selected?.semester ?? ""); setColor(selected?.color ?? "#8fd6c2"); setError("");
  }, [selected]);
  ...
  async function save(event: FormEvent) {
    ...
    try {
      ...
      setSelected(null); setName(""); setCode(""); setInstructor(""); setSchedule(""); setSemester(""); setColor("#8fd6c2");
      router.refresh();
  ...
  async function action(subject: AssignmentSubject, actionName: "archive" | "restore" | "delete") {
    if (actionName === "delete" && !(await confirmAction(`Permanently delete “${subject.name}”?`))) return;
    setBusy(true); setError("");
    try {
  ...
  return <ModalPortal><AnimatePresence>{open ? (
    <motion.div className="tech-modal-overlay fixed inset-0 z-[100] grid place-items-center overflow-y-auto p-3 sm:p-5" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
      <motion.div className="tech-modal-surface max-h-[94dvh] w-full max-w-5xl overflow-hidden rounded-[28px] border" initial={{ opacity: 0, y: 18, scale: .97 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: 10, scale: .98 }}>
        <header className="flex items-start gap-4 border-b border-white/10 p-5 sm:p-6"><span className="grid size-11 place-items-center rounded-2xl bg-primary/10 text-primary"><Settings2 className="size-5" /></span><div className="flex-1"><h2 className="text-lg font-semibold text-slate-100">Manage subjects</h2><p className="mt-1 text-sm text-slate-400">Create, edit, archive, and restore assignment subjects.</p></div><button type="button" onClick={onClose} aria-label="Close subject manager" className="grid size-10 place-items-center rounded-lg text-slate-400 hover:bg-white/[0.06]"><X className="size-5" /></button></header>
        <div className="grid max-h-[calc(94dvh-94px)] overflow-auto lg:grid-cols-[1.05fr_.95fr]">
          <div className="border-b border-white/10 p-5 lg:border-b-0 lg:border-r sm:p-6"><div className="space-y-2">{subjects.length ? subjects.map((subject) => <div key={subject.id} className="flex items-center gap-3 rounded-2xl border border-white/10 p-3"><span className="size-3 rounded-lg" style={{ backgroundColor: subject.color }} /><div className="min-w-0 flex-1"><strong className="block truncate text-sm text-slate-100">{subject.name}</strong><small className="text-xs text-slate-400">{subject.code || "No code"}{subject.is_archived ? " · Archived" : ""}</small></div><button type="button" aria-label={`Edit ${subject.name}`} onClick={() => setSelected(subject)} className="grid size-9 place-items-center rounded-lg hover:bg-white/[0.06]"><Pencil className="size-4" /></button><button type="button" aria-label={`${subject.is_archived ? "Restore" : "Archive"} ${subject.name}`} disabled={busy} onClick={() => action(subject, subject.is_archived ? "restore" : "archive")} className="grid size-9 place-items-center rounded-lg hover:bg-white/[0.06]">{subject.is_archived ? <RotateCcw className="size-4" /> : <Archive className="size-4" />}</button><button type="button" aria-label={`Delete ${subject.name}`} disabled={busy} onClick={() => action(subject, "delete")} className="grid size-9 place-items-center rounded-lg text-red-300 hover:bg-red-400/10"><Trash2 className="size-4" /></button></div>) : <p className="rounded-2xl bg-white/[0.035] p-5 text-sm text-slate-400">No subjects yet.</p>}</div></div>
          <form onSubmit={save} className="space-y-4 p-5 sm:p-6"><div className="flex items-center gap-2"><Plus className="size-5 text-primary" /><h3 className="font-semibold text-slate-100">{selected ? "Edit subject" : "New subject"}</h3></div><Field label="Name"><input value={name} onChange={(e) => setName(e.target.value)} required maxLength={100} className={inputClass} /></Field><div className="grid gap-4 sm:grid-cols-2"><Field label="Code"><input value={code} onChange={(e) => setCode(e.target.value)} maxLength={30} className={inputClass} /></Field><Field label="Color"><input type="color" value={color} onChange={(e) => setColor(e.target.value)} className={`${inputClass} p-2`} /></Field></div><Field label="Instructor"><input value={instructor} onChange={(e) => setInstructor(e.target.value)} maxLength={100} className={inputClass} /></Field><Field label="Schedule"><input value={schedule} onChange={(e) => setSchedule(e.target.value)} maxLength={255} className={inputClass} /></Field><Field label="Semester"><input value={semester} onChange={(e) => setSemester(e.target.value)} maxLength={100} className={inputClass} /></Field>{error ? <div className="rounded-2xl border border-red-300/25 bg-red-400/10 p-3 text-sm text-red-300">{error}</div> : null}<div className="flex gap-2"><button type="submit" disabled={busy} className="inline-flex min-h-11 flex-1 items-center justify-center gap-2 rounded-lg workspace-primary px-5 text-sm font-semibold text-white disabled:opacity-60">{busy ? <Loader2 className="size-4 animate-spin" /> : null}{selected ? "Save subject" : "Add subject"}</button>{selected ? <button type="button" onClick={() => setSelected(null)} className="min-h-11 rounded-lg border border-white/10 px-5 text-sm font-semibold">Cancel</button> : null}</div></form>
        </div>
      </motion.div>
    </motion.div>
...
const inputClass = "min-h-11 w-full rounded-2xl border border-white/10 bg-white/[0.045] px-4 text-sm text-slate-100 outline-none focus:border-primary/45 focus:ring-4 focus:ring-primary/15";
