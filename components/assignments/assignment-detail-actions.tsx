...
import { AlarmClock, Archive, CheckCircle2, CircleDot, Loader2, Pencil, Trash2 } from "@/components/ui/icons";
...
export function AssignmentDetailActions({ assignment, subjects }: { assignment: AssignmentItem; subjects: AssignmentSubject[] }) {
  ...
  async function snooze(minutes: number) {
    ...
    try {
      ...
      (await showNotice(error instanceof Error ? error.message : "The reminder could not be snoozed."));
    } finally {
      setBusy(false);
    }
  }
  ...
  return <>
    <div className="flex flex-wrap items-center gap-2">
      <button type="button" onClick={() => setEditing(true)} className="inline-flex min-h-10 items-center gap-2 rounded-full workspace-primary px-4 text-sm font-semibold text-white hover:brightness-110"><Pencil className="size-4" />Edit</button>
      ...
      <button type="button" onClick={() => snooze(60)} disabled={busy || assignment.status === "done" || assignment.status === "submitted"} className="inline-flex min-h-10 items-center gap-2 rounded-full border border-primary/20 bg-white/[0.045] px-4 text-sm font-semibold text-primary hover:bg-primary/10 disabled:opacity-50"><AlarmClock className="size-4" />Snooze 1 hour</button>
