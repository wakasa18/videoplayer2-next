...
import { Archive, ArrowLeft, Loader2, RotateCcw, Trash2 } from "@/components/ui/icons";
...
export function AssignmentCollection({
  ...
  async function itemAction(id: number, action: "restore" | "unarchive" | "delete") {
    if (action === "delete" && !(await confirmAction("Permanently delete this assignment and its subtasks, notes, and links?"))) return;
    setBusyId(id);
    try {
  ...
  async function bulk(action: "restore" | "delete") {
    ...
    if (action === "delete" && !(await confirmAction(`Permanently delete ${selected.length} assignments?`))) return;
    setBulkBusy(true);
    try {
      ...
      (await showNotice(error instanceof Error ? error.message : "The assignments could not be updated."));
    } finally {
      setBulkBusy(false);
    }
  }
  ...
  return (
    <main className="space-y-5">
      <Link href="/dashboard/assignments" className="inline-flex items-center gap-2 rounded-full px-3 py-2 text-sm font-semibold text-primary hover:bg-primary/10"><ArrowLeft className="size-4" />Back to assignments</Link>
          ...
          {result.assignments.map((assignment, index) => (
            <div key={assignment.id} className="rounded-[22px]">
              <div className="relative [&_a]:pointer-events-none">
                {isRecycle ? (
                  <label className="absolute left-4 top-4 z-20 grid size-9 cursor-pointer place-items-center rounded-full bg-card/95 shadow-sm hover:bg-white/[0.06]">
                    <input
                      type="checkbox"
              ...
              <div className="mt-2 flex flex-wrap items-center justify-end gap-2">
                ...
                {busyId === assignment.id ? <Loader2 className="size-4 animate-spin text-primary" /> : null}
              </div>
            </div>
          ))}
        </section>
