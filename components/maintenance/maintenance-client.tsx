...
export function MaintenanceClient({ data }: { data: MaintenanceDashboardData }) {
  ...
  async function clearReviewedErrors() {
    const confirmed = (await confirmAction("Clear all reviewed application error logs? This does not change files, videos, or assignments."));
    if (!confirmed) return;
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="grid gap-5 xl:grid-cols-[minmax(0,1.15fr)_minmax(340px,.85fr)]">
        <section className="tech-card rounded-[24px] border border-white/10 bg-white/[0.045] p-5 shadow-sm sm:p-6">
          <div className="flex items-center gap-3"><span className="grid size-11 place-items-center rounded-2xl bg-primary/10 text-primary"><Database className="size-5" /></span><div><h2 className="text-lg font-semibold text-slate-100">Operational snapshot</h2><p className="text-sm text-slate-400">Database, Storage, and error-history totals.</p></div></div>
          ...
          <div className="mt-5">
            ...
            <div className="mt-2 h-2.5 overflow-hidden rounded-full bg-white/[0.08]"><div className="h-full rounded-full workspace-primary" style={{ width: `${Math.min(100, current.storage.quotaPercent)}%` }} /></div>
          </div>
        </section>

        <section className="tech-card rounded-[24px] border border-white/10 bg-white/[0.045] p-5 shadow-sm sm:p-6">
          <div className="flex items-center gap-3"><span className="grid size-11 place-items-center rounded-2xl bg-primary/10 text-primary"><ArchiveRestore className="size-5" /></span><div><h2 className="text-lg font-semibold text-slate-100">Backup and recovery</h2><p className="text-sm text-slate-400">Download and verify metadata backups.</p></div></div>
      ...
      <section className="tech-card rounded-[24px] border border-white/10 bg-white/[0.045] p-5 shadow-sm sm:p-6">
        <div className="flex items-center gap-3">
          <span className="grid size-11 place-items-center rounded-2xl bg-primary/10 text-primary"><ShieldCheck className="size-5" /></span>
          <div>
        ...
        {current.recentErrors.length ? (
          <div className="mt-5 grid gap-3">
            {current.recentErrors.map((error) => (
              <article key={`${error.id}-${error.digest ?? error.message}`} className="tech-card rounded-2xl border border-white/10 bg-white/[0.025] p-4">
      ...
      <div className="grid gap-5 xl:grid-cols-2">
        <section className="tech-card rounded-[24px] border border-white/10 bg-white/[0.045] p-5 shadow-sm sm:p-6">
          <div className="flex items-center gap-3"><Gauge className="size-5 text-primary" /><h2 className="text-lg font-semibold text-slate-100">Timed checks</h2></div>
...
function SummaryCard({ icon: Icon, label, value, detail, status }: { icon: typeof ShieldCheck; label: string; value: string; detail?: string; status: MaintenanceStatus }) {
  return <article className="tech-card rounded-[22px] border border-white/10 bg-white/[0.045] p-5 shadow-sm"><div className="flex items-center justify-between"><Icon className="size-5 text-primary" /><StatusDot status={status} /></div><span className="mt-4 block text-sm font-medium text-slate-400">{label}</span><strong className="mt-1 block text-2xl font-semibold text-slate-100">{value}</strong>{detail ? <p className="mt-1 text-xs text-slate-400">{detail}</p> : null}</article>;
}
...
const primaryButton = "inline-flex items-center justify-center gap-2 rounded-full workspace-primary px-5 py-2.5 text-sm font-semibold text-white transition hover:workspace-primary disabled:opacity-60";
