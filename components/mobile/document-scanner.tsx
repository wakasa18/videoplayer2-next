...
import { Camera, CheckCircle2, Download, FileText, ImagePlus, Loader2, ScanLine, Trash2, X } from "@/components/ui/icons";
    ...
    <AnimatePresence>
      {open ? (
        ...
        >
          ...
          >
            <header className="flex items-start gap-3"><span className="grid size-11 place-items-center rounded-xl bg-primary/10 text-primary"><ScanLine className="size-5" /></span><div className="min-w-0 flex-1"><h2 className="text-lg font-semibold text-slate-100">Camera Document Scanner</h2><p className="mt-1 text-xs leading-5 text-slate-400">Capture pages, auto-trim document edges, enhance readability, and save one PDF.</p></div><motion.button type="button" whileTap={{ scale: .9 }} onClick={onClose} disabled={busy} aria-label="Close document scanner" className="grid size-10 place-items-center rounded-xl border border-white/10 text-slate-400"><X className="size-5" /></motion.button></header>
            <div className="mt-5 grid grid-cols-2 gap-2"><motion.button type="button" whileTap={{ scale: .97 }} onClick={() => inputRef.current?.click()} className="min-h-12 rounded-xl workspace-primary px-3 text-sm font-semibold text-white"><Camera className="mr-2 inline size-4" />Scan page</motion.button><motion.button type="button" whileTap={{ scale: .97 }} onClick={() => galleryRef.current?.click()} className="min-h-12 rounded-xl border border-white/10 bg-white/[.045] px-3 text-sm font-semibold text-slate-200"><ImagePlus className="mr-2 inline size-4" />Choose photos</motion.button></div>
            ...
            <div className="mt-4 rounded-2xl border border-white/10 bg-white/[.03] p-3"><div className="flex items-center justify-between gap-3"><div><strong className="text-sm text-slate-200">Pages</strong><p className="text-xs text-slate-500">{pages.length} page{pages.length === 1 ? "" : "s"} · {totalMb.toFixed(1)} MB source</p></div>{pages.length ? <motion.button type="button" whileTap={{ scale: .9 }} onClick={() => { setPages([]); setResult(null); }} className="grid size-9 place-items-center rounded-lg text-red-300 hover:bg-red-400/10"><Trash2 className="size-4" /></motion.button> : null}</div>{pages.length ? <div className="mt-3 flex gap-2 overflow-x-auto pb-1">{pages.map((file, index) => <motion.div initial={{ opacity: 0, scale: .96 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: .16 }} key={`${file.name}-${index}`} className="min-w-24 rounded-xl border border-white/10 bg-card p-2"><FileText className="size-5 text-primary" /><p className="mt-2 truncate text-[10px] text-slate-300">Page {index + 1}</p></motion.div>)}</div> : <p className="mt-3 text-xs text-slate-500">No pages captured yet.</p>}</div>
            <label className="mt-4 flex items-center gap-3 rounded-xl border border-white/10 bg-white/[.03] p-3 text-sm text-slate-300"><input type="checkbox" checked={enhance} onChange={(event) => setEnhance(event.target.checked)} className="size-4 accent-primary" /><span><strong className="block text-slate-200">Enhance readability</strong><span className="text-xs text-slate-500">Auto-crop, grayscale, and boost contrast.</span></span></label>
            <label className="mt-4 block text-xs font-semibold text-slate-400">PDF filename<input value={name} onChange={(event) => setName(event.target.value)} className="mt-2 h-11 w-full rounded-xl border border-white/10 bg-card px-3 text-sm text-slate-100 outline-none focus:border-primary/30" /></label>
            <AnimatePresence initial={false}>{status ? <motion.div initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -4 }} transition={{ duration: .16 }} className="mt-4 flex items-start gap-2 rounded-xl border border-primary/15 bg-primary/[.06] p-3 text-xs leading-5 text-slate-300">{status.includes("saved") || status.includes("Saved") ? <CheckCircle2 className="mt-0.5 size-4 shrink-0 text-emerald-300" /> : null}{status}</motion.div> : null}</AnimatePresence>
            <div className="mt-5 flex flex-wrap justify-end gap-2">{result ? <motion.button type="button" whileTap={{ scale: .97 }} onClick={downloadResult} className="min-h-11 rounded-lg border border-white/10 px-4 text-sm font-semibold text-slate-200"><Download className="mr-2 inline size-4" />Download copy</motion.button> : null}<motion.button type="button" whileTap={{ scale: .97 }} disabled={!pages.length || busy} onClick={() => void buildAndSave()} className="min-h-11 rounded-lg workspace-primary px-5 text-sm font-semibold text-white disabled:opacity-50">{busy ? <Loader2 className="mr-2 inline size-4 animate-spin" /> : <ScanLine className="mr-2 inline size-4" />}Create & save PDF</motion.button></div>
          </motion.section>
        </motion.div>
      ) : null}
    </AnimatePresence>
