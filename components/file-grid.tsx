  ...
  return (
    <div>
      <div className="mb-3 flex justify-end">
        <button type="button" onClick={() => setSelected(selected.size === files.length ? new Set() : new Set(files.map((file) => file.id)))} className="inline-flex min-h-9 items-center gap-2 rounded-xl border border-white/10 bg-white/[.035] px-3 text-xs font-semibold text-slate-400 hover:bg-white/[.06] hover:text-slate-200"><CheckSquare className="size-4" />{selected.size === files.length ? "Clear selection" : "Select all on page"}</button>
      </div>
