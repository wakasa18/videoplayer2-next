...
import { CheckCircle2, FileJson, Loader2, RotateCcw, ShieldCheck } from "@/components/ui/icons";
...
export function BackupRestoreTools() {
  ...
  async function submit(mode: "validate" | "merge") {
    if (!file || busy) return;
    if (mode === "merge" && !(await confirmAction("Restore metadata from this backup? This safe merge updates matching records only and does not restore Storage bytes or credentials."))) return;
  ...
  return <section className="tech-card rounded-[24px] border border-white/10 bg-white/[0.045] p-5 shadow-sm sm:p-6">
    <div className="flex items-start justify-between gap-4"><div><h2 className="text-lg font-semibold text-slate-100">Backup restore</h2><p className="mt-1 text-sm leading-6 text-slate-400">Validate a Damon’s Archive JSON backup, then safely merge metadata back into existing records.</p></div><span className="grid size-11 shrink-0 place-items-center rounded-2xl bg-primary/10 text-primary"><RotateCcw className="size-5" /></span></div>
    <button type="button" onClick={() => inputRef.current?.click()} className="mt-5 flex min-h-12 w-full items-center gap-3 rounded-2xl border border-dashed border-primary/25 bg-primary/[.035] px-4 text-left hover:bg-primary/[.055]"><FileJson className="size-5 text-primary" /><span className="min-w-0"><strong className="block truncate text-sm text-slate-200">{file?.name || "Choose JSON backup"}</strong><small className="text-xs text-slate-500">Metadata restore only — Storage bytes and secrets stay untouched</small></span></button>
